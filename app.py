...
# --- 2. UI STYLING (Blue Buttons + Floating Digit) ---
st.markdown("""
    <style>
    ...
    with col_a:
        if st.button("Start Scoring", use_container_width=True):
            try:
                ...
                st.error("Connection error. Check Sheet 'Anyone with link' settings.")
...
# --- 6. REVIEW & SUBMIT ---
    ...
    if st.button("🏁 FINISH & SUBMIT", type="primary"):
        ...
        try:
            ...
            r = requests.post(FORM_URL, data=payload, headers={'Content-Type': 'application/x-www-form-urlencoded'})
            if r.status_code == 200:
                st.balloons()
                st.session_state.step = "done"
                st.rerun()
            else:
                st.error(f"Error {r.status_code}. Please check Form 'Restriction' settings.")
        except Exception as e:
            st.error(f"Failed: {e}")

elif st.session_state.step == "done":
    st.title("⛳ Round Complete!")
    st.success("Your scores have been submitted successfully.")
